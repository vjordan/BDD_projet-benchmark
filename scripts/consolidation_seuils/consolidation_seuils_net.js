...
const measurement = 'net';
const measurement_target = 'net_thresholds';
...
for (let offset = 0; offset < offset_max; offset++) {
  ...
  for (seuil of seuils) {
const requete = `SELECT COUNT("droppedrx") AS "net_dropped_${seuil}" INTO "${measurement_target}" FROM "${measurement}" WHERE ("droppedrx" + "droppedtx" >= ${seuil}) AND (time >= ${timestamp_debut}000000000 + ${offset}h AND time <= ${timestamp_debut}000000000 + ${offset + 1}h) GROUP BY uuid,time(1h)`;
