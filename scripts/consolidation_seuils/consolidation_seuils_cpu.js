...
const measurement_target = 'cpu_thresholds';
const champ = '"usage"';
...
for (let offset = 0; offset < offset_max; offset++) {
  ...
  for (seuil of seuils) {
    const requete = `SELECT COUNT(${champ}) AS "cpu_${seuil}" INTO "${measurement_target}" FROM "${measurement}" WHERE "usage" >= ${seuil} AND (time >= ${timestamp_debut}000000000 + ${offset}h AND time <= ${timestamp_debut}000000000 + ${offset + 1}h) GROUP BY uuid,time(1h)`;
